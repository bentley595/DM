...
func _on_name_submitted(text: String) -> void:
	...
	# Unlock the first ingredient so the player can enter the dungeon
	# immediately!  More ingredients unlock by playing (see ingredient_data.gd).
	get_tree().set_meta("unlocked_ingredients", ["slime_essence"])
